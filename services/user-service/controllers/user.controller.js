  ...
  try {
    ...
    if (existingProfile.rows.length > 0) {
      ...
      query = `
        ...
        SET name = $1, education_level = $2, experience_level = $3, preferred_roles = $4
            ${cv_url ? ', cv_url = $5' : ''}
        WHERE account_id = ${cv_url ? '$6' : '$5'}
        RETURNING *
      `;
      values = cv_url 
        ? [name, education_level, experience_level, JSON.stringify(rolesArray), cv_url, userId]
        : [name, education_level, experience_level, JSON.stringify(rolesArray), userId];
    } else {
      // Create new profile
      query = `
        INSERT INTO devconnect.users 
        (account_id, name, education_level, experience_level, preferred_roles${cv_url ? ', cv_url' : ''})
        VALUES ($1, $2, $3, $4, $5${cv_url ? ', $6' : ''})
        RETURNING *
      `;
      values = cv_url 
        ? [userId, name, education_level, experience_level, JSON.stringify(rolesArray), cv_url]
        : [userId, name, education_level, experience_level, JSON.stringify(rolesArray)];
    }
